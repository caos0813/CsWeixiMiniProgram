!function(modules) {
    ...
    var detectReact = "\nwindow.__REACT_DEVTOOLS_GLOBAL_HOOK__.on('renderer', function(evt) {\n  window.postMessage({\n    source: 'react-devtools-detector',\n    reactBuildType: evt.reactBuildType,\n  }, '*');\n});\n"
      ...
      , js = ";(" + installGlobalHook.toString() + "(window));(" + installRelayHook.toString() + "(window))" + saveNativeValues + detectReact
        ...
        function detectReactBuildType(renderer) {
            try {
                if ("string" == typeof renderer.version)
                    return renderer.bundleType > 0 ? "development" : "production";
              ...
              , hook = {
                ...
                checkDCE: function(fn) {
                    try {
                        ...
                        code.indexOf("^_^") > -1 && (hasDetectedBadDCE = !0,
                        setTimeout(function() {
                ...
                on: function(evt, fn) {
                    hook._listeners[evt] || (hook._listeners[evt] = []),
                    hook._listeners[evt].push(fn);
                },
                off: function(evt, fn) {
                    if (hook._listeners[evt]) {
                        var ix = hook._listeners[evt].indexOf(fn);
                        ix !== -1 && hook._listeners[evt].splice(ix, 1),
                        hook._listeners[evt].length || (hook._listeners[evt] = null);
                    }
                },
                ...
                getFiberRoots: function(rendererID) {
                    ...
                    return roots[rendererID] || (roots[rendererID] = new Set()),
                    roots[rendererID];
                },
...
, function(module, exports) {
    ...
    function installRelayHook(window) {
        ...
        function instrumentRelayRequests(relayInternals) {
            ...
            return instrumented.setRequestListener = setRequestListener,
            instrumented;
        }
